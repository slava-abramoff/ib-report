...
import fs from 'fs';
import { Event, Incident } from '@prisma/client';
...
export function generateIncidentsPDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      doc.font('Times-Roman'); // fallback, если шрифт недоступен
    }
    ...
    doc.text(
      `Дата и время сообщения: ${incident.reportDateTime ?? emptyString}`,
    );
    doc.moveDown(1);
...
export function generateEventsPDF(
  event: Event,
  outputPath: string,
): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(16)
      .text('Отчет об инциденте информационной безопасности', {
        align: 'center',
      });
    doc.moveDown(1.5);
    ...
    doc.moveDown(0.5);
    doc.text(
      `Фамилия ${event.surname ?? emptyString}    Адрес ${event.address ?? emptyString}`,
    );
    doc.text(
    ...
    doc.text(
      `Пораженные компоненты ${event.affectedComponents ?? emptyString}`,
    );
    doc.text(
    ...
    doc.text(
      `Дата и время обнаружения инцидента ${event.detect ?? emptyString}`,
    );
